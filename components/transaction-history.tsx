...
import { Search, Filter, X, TrendingUp, TrendingDown, Download } from "lucide-react"
...
export function TransactionHistory({
  ...
  const exportToCSV = (transactions: Transaction[], exportTitle: string) => {
    try {
      const headers = ['Date', 'Item', 'Size', 'Brand', 'Type', 'Quantity', 'Balance']
      const csvData = [
        headers.join(','),
  ...
  const exportToJSON = (transactions: Transaction[], exportTitle: string) => {
    try {
      const exportData = {
        metadata: {
          title: exportTitle,
          ...
          generatedBy: "Flux Stock Management System",
          poweredBy: "TekvoLabs",
          website: "https://tekvolabs.com/",
          filters: {
            searchTerm: searchTerm || null,
            selectedItem: selectedItem !== "all" ? selectedItem : null,
        ...
        transactions: transactions.map(transaction => ({
          ...
          size: transaction.sizes.name,
          brand: transaction.brand,
          type: transaction.received_quantity > 0 ? 'Received' : 'Issued',
          quantity: transaction.received_quantity > 0 ? transaction.received_quantity : transaction.issued_quantity,
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `flux-${exportTitle.toLowerCase().replace(/\s+/g, '-')}-${format(new Date(), "yyyy-MM-dd")}.json`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)
      ...
      <CardHeader className="bg-gradient-to-r from-gray-50 to-white border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={() => exportToJSON(filteredTransactions, title)}
              disabled={filteredTransactions.length === 0}
              size="sm"
              variant="outline"
              className="bg-white hover:bg-gray-50 border-gray-300 text-gray-700"
